...
def split_train_test(X, y, Tp):
    ...
    for i in range(len(X)):
        ...
        X1_train, X1_test, y1_train, y1_test = train_test_split(
            X1, y1, train_size=Tp/100., shuffle=True)
...
def plot_FD(fig, ax, clf, show_SV=False):
    ...
    X1, X2 = np.meshgrid(np.linspace(xmin, xmax, 100),
...
class PerceptronKernel():
    ...
    def kernel(self, *args):
        try:
            ...
            print(
                "ERROR: Debes pasar al menos un argumento con la etiqueta del kernel: {lk,gk,pk}")
            ok = False
        if ok:
            if kernel == 'lk':
                self.__kernel = self.__lk
    ...
    def fit(self, X, y):
        ...
        print("{0} vectores de soporte de {1} puntos".format(
            len(self.alpha), n_samples))
